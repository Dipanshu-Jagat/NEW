...
<table id = "table" class = "table table-bordered">
	...
	<tbody>
		<?php
			...
			$q_transact = $conn->query("SELECT * FROM `transaction` LEFT JOIN `expense` ON transaction.transact_detail = expense.expense_id WHERE `student_id` = '$_SESSION[student_id]' && `status` = 'Balance' ORDER BY `ay` ") or die(mysqli_error());
